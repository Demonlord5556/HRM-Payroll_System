...
const login = async (req, res) => {
  try {
    // ==========================================
    // DEBUG LOGS MOVED TO THE VERY TOP
    // ==========================================
    console.log("\n=== NEW LOGIN ATTEMPT ===");
    console.log("Incoming data from browser:", req.body);
    ...
    console.log(`Database found ${users.length} matching user(s).`);
    console.log("=========================\n");
    // ==========================================
...
const getCurrentUser = async (req, res) => {
  try {
    ...
    const [unreadLeaves] = await pool.query(
      'SELECT COUNT(*) as count FROM leave_requests WHERE employee_id = ? AND status = ? AND approved_by IS NOT NULL AND rejection_reason IS NOT NULL',
      [row.employee_record_id, 'Rejected']
    ).catch(() => [{ count: 0 }]);

    const [unreadDocs] = await pool.query(
      'SELECT COUNT(*) as count FROM documents WHERE employee_id = ? AND is_verified = 0',
